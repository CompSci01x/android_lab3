...
public class RecipeSearchAsyncTask extends AsyncTask<String, Void, RecipeModel> {
    ...
    protected RecipeModel doInBackground(String... params) {
        ...
        HttpUrl.Builder urlBuilder = HttpUrl.parse(baseApiUrl).newBuilder(); urlBuilder.addQueryParameter("_app_key", apiKey); urlBuilder.addQueryParameter("_app_id", appId); urlBuilder.addQueryParameter("your_search_parameters", searchParams); String url = urlBuilder.build().toString();
        Request request = new Request.Builder().url(url).build();
        Response response = null;

        try {
// response holds server's answer
            response = client.newCall(request).execute(); if (response != null) {
